...
def build_emcees_table(ft_battles: pd.DataFrame) -> pd.DataFrame:
    """
...
def write_emcees_table(ft_battles: pd.DataFrame, out_path: PathLike) -> None:
    """Build the emcee table from ft_battles and write it to CSV."""
    build_emcees_table(ft_battles).to_csv(out_path, index=False)
...
def build_battle_network(ft_battles: pd.DataFrame) -> nx.Graph:
    """
